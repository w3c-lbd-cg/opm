...
class GeneralQueries {
    ...
    getResourceNamedGraph(resourceURI) {
        return `SELECT DISTINCT ?g WHERE { GRAPH ?g {<${resourceURI}> ?p ?o}}`;
    }
    addObjectProperty(subjectURI, predicateURI, objectURI, graphURI) {
        return `INSERT DATA {GRAPH <${graphURI}> {<${subjectURI}> <${predicateURI}> <${objectURI}>}}`;
    }
    addProjectData(uri, name, description) {
        var q = '';
    ...
    checkIfPropertyExistOnResource(resourceURI, propertyURI) {
        return `ASK WHERE {GRAPH ?g {<${resourceURI}> <${propertyURI}> <?o>}}`;
    }
    wipeNamedGraph(graphURI) {
...
exports.GeneralQueries = GeneralQueries;
